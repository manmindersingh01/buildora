interface CodeFile {
  ...
  function parseFrontendCode(input: string): ParsedResult {
    try {
      ...
      const codeFiles: CodeFile[] = Object.entries(data.codeFiles).map(([path, content]) => ({
        path,
  ...
  function flattenStructure(structure: StructureNode, basePath: string = ''): string[] {
    const paths: string[] = [];
  ...
  function getFileStatus(structure: StructureNode, filePath: string): string | null {
  ...
  function getCodeFileByPath(codeFiles: CodeFile[], path: string): CodeFile | null {
